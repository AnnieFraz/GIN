...
public class TestRunner {
    ...
    protected boolean compile(String className, String source, CacheClassLoader classLoader)  {
        ...
        try {
            ...
            //CompiledCode code = compiler.compile(className, source);
            ...
            if (e.getMessage().contains("does not exist")) {
                System.out.println("Did you set the classpath with -cp=?");
            }

        } catch (Exception e) {
            System.out.println("Unrecognised exception compiling class " + className);
            System.out.println(e.getMessage());
            System.exit(-1);
        }
